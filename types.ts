...
export interface RoutineSlot {
    ...
    activityType: 'LEARN' | 'REVISION' | 'PRACTICE' | 'TEST' | 'CATCH_UP';
    topic?: string; // "Algebra Basics"
    isCompleted: boolean;
    completedAt?: string;
    isMissed?: boolean;
    isCustom?: boolean; // Added by student
}

export interface StudyRoutine {
    streak: number;
    bonusHolidays: number; // Earned by >25 days/month
    lastStudyDate: string; // YYYY-MM-DD
    missedSlots: RoutineSlot[]; // For Sunday Catch-up
    customSlots: RoutineSlot[]; // Student added slots
    dailyStats: Record<string, number>; // Date -> Minutes Studied
    preferences: {
        startTime: string; // "06:00" default
        slotsPerDay: number; // 6 default
        difficultyRatings: Record<string, 'EASY'|'MEDIUM'|'HARD'>; // Topic ID -> Difficulty
    };
}
